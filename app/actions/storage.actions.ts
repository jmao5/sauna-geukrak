...
/**
 * 여러 장 병렬 업로드 (Server Action) — 일반 사우나 사진용
 */
...
/**
 * 사우나 내부 모형도/도면 이미지 업로드 (Server Action)
 * Storage 경로: floor-plans/{saunaId}/{uuid}.{ext}
 */
export async function uploadFloorPlanAction(saunaId: string, formData: FormData): Promise<string[]> {
  ...
  const results = await Promise.allSettled(
    files.map(async (file) => {
      const ext = file.name.split('.').pop() ?? 'jpg'
      const path = `floor-plans/${saunaId}/${crypto.randomUUID()}.${ext}`
      const { error } = await supabase.storage
        .from('sauna-geukrak')
        .upload(path, file, { upsert: false, contentType: file.type })
      if (error) throw new Error(path)
